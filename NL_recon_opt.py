...
path = savedata.folder.create('NL_recon_opt')
...
# ngen = [100, 200, 500, 1000, 3000, 5000, 30000]
pop_new = algo.opt(pop_new, ngen, path)
...
for ind in pop:
    x = np.delete(ind.best_x, id_down_2)
    prob.recreate_pop6(x)
    pop_new.push_back(x)
plt.figure()
cur_f = np.array([ind.cur_f for ind in pop_new]).T
plt.scatter(cur_f[0], cur_f[1], c='r', label = str(n_off)+' Cav. off')
sav.save_pop(path+'/devsort_'+str(n_off)+'.nl', pop_new)
...
plt.ylabel('Trip Rate [per hour]')
plt.legend()
plt.savefig(path+'/'+'PF_reconstruct_devsort_'+str(n_off)+'.eps', format="eps")
# plt.show()
plt.close()
