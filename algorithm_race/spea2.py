...
def race(pop, ngen, m, cr, path):
    ...
    for _, mi in enumerate(m):
        for _, cri in enumerate(cr):
            print(mi, cri)
            pop_race = pop
            algo = algorithm.spea2(ngen, m=mi, cr=cri)
            pop_race = algo.evolve(pop_race)
            save_pop(path + '/pop_spea2_' + str(ngen)+'_m_'+'{:.3f}'.format(mi)+'_cr_'+'{:.3f}'.format(cri), pop_race)
    ...
    plt.title('spea2, '+str(ngen)+' generations')
    plt.ylabel("Trip Rate [per hour]")
    plt.xlabel("Heat Load [W]")
    plt.savefig(path+'/'+'spea2_race_ngen_'+str(ngen)+'_'+date_str+'.eps', format="eps")
    plt.show()
    return pop