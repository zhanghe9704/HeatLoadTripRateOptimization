...
# ax1.annotate('(a) North Linac', xytext=(2813,2), xy=(2813,2))
ax1.legend(loc="upper right", handletextpad=0.2, labelspacing=-0.0)
ax1.grid()

time_tag = '2017_04_07__17_11_42'
...
# ax2.set_xticks(np.arange(2800,4200+1,250))
ax2.set_xlabel('Heat load [W]')
ax2.set_ylabel('Trips [per hour]')
ax2.set_title('(b) South Linac')
# ax2.annotate('(b) South Linac', xytext=(2860,1.5), xy=(2860,1.5))
ax2.legend(loc="upper right", handletextpad=0.2, labelspacing=-0.0)
ax2.grid()
plt.tight_layout()
path = savedata.folder.create('plots')
plt.savefig(path+'/reopt_scale_plus.eps', format="eps")
plt.savefig(path+'/reopt_scale_plus.pdf', format="pdf")
plt.savefig(path+'/reopt_scale_plus.png', format="png")
plt.show()